...
An offline-first Expo/React Native devotional app for Seventh-day Adventists — Bible
(multi-translation, offline), hymnals, Ellen G. White books, Bible commentary, Sabbath
School lessons, devotionals, prayer, notes, health tracking, and reminders.
...
| Sabbath School lessons (auto-synced) | `app/(tabs)/more/sabbath-school*` | `database/sabbathSchool.ts`, `services/sabbathSchoolSync.ts` |
...
| Notes (with per-note reminders) | `app/(tabs)/notes/` | `database/notes.ts` |
...
| Reminders/notifications | `app/(tabs)/more/settings.tsx` | `database/reminders.ts`, `services/notifications.ts` |
