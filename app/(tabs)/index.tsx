...
import { getGreeting, formatLongDate } from '@/utils/greeting';
...
function TimeOfDayIcon({ color }: { color: string }) {
  ...
  if (hour < 5 || hour >= 20) return <Moon size={22} color={color} strokeWidth={1.75} />;
  if (hour < 11) return <Sunrise size={22} color={color} strokeWidth={1.75} />;
  if (hour < 17) return <Sun size={22} color={color} strokeWidth={1.75} />;
